	...
	return async function({ event }) {
		...
		switch (event.logMessageType) {
			...
			case "log:unsubscribe":
				...
				else api.sendMessage(`${/đã xóa (.*?) khỏi nhóm/.exec(event.logMessageBody)[1]} vừa bị đá khỏi nhóm CÔNG TY SUPERCLOUDSERVER WEBSITE: HTTPS://SuperCloudServer.Online/Facebook-InFo🤔`, event.threadID);
				break;
